...
interface User {
  id: string;
  name: string | null; // Make name nullable to match Prisma
  email: string;
  role: string; // Keep role as required
}
...
interface RawMaterial {
  ...
  name: string;
  quantity: number;
  unit: string | null; // Updated to match Prisma schema
}
...
export function RawMaterialTransfer({
  ...
  const addItem = () => {
    ...
    if (selectedMaterial.quantity < currentItem.quantityIssued) {
      alert(
        `Insufficient stock! Available: ${selectedMaterial.quantity} ${
    ...
    const newItem: TransferItem = {
      ...
      materialUnit: selectedMaterial.unit || "pcs", // Provide default if null
  ...
  const getSelectedUserName = () => {
    ...
    return user?.name || user?.email || "Select user";
  };
